...
class AsociadorFase1:
    ...
    def __init__(self, umbral: float = 0.6, metodo: str = "ponderado"):
        ...
        logger.info(f"AsociadorFase1 inicializado: umbral={umbral}, metodo={metodo}")
    ...
    def registrar_sesion(self, asistentes: List[str]):
        ...
        logger.debug(
            f"Sesión {self.total_sesiones} registrada: {len(asistentes)} asistentes"
        )
    ...
    def sugerir_grupo(
        self, persona: str, min_confianza: float = None
    ...
    def descubrir_clases(
        self, min_confianza: float = None, min_miembros: int = 2
    ) -> List[Set[str]]:
    ...
    def metricas_calidad(self) -> Dict:
        ...
        return {
            ...
            "porcentaje_confiables": (
                (pares_confiables / total_pares * 100) if total_pares > 0 else 0
            ),
            "confianza_promedio": (
                suma_confianzas / total_pares if total_pares > 0 else 0
            ),
        }
